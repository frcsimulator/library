...
public class DriverStationLCD extends SensorBase implements IInputOutput{
    ...
    byte[] m_textBuffer;
    String[] m_stringBuffer = new String[6];
	...
	String send = "";
	for(int i = 0; i < m_stringBuffer.length; i++){
	    send += m_stringBuffer[i] + "\n";
	}
	byte[] data = new byte[send.length()];
	for(int i = 0; i < data.length; i++){
	    data[i] = (byte)send.charAt(i);
	}
        FRCControl.setUserDsLcdData(data, FRCControl.USER_DS_LCD_DATA_SIZE, kSyncTimeout_ms);
    }
	...
	String tmp = "";
	    ...
	    String processed = "";
	    for(int i = 0; i < 21; i++){
		processed += tmp.charAt(i);
	    }
	    m_stringBuffer[line.value] = processed;
        }
    }
