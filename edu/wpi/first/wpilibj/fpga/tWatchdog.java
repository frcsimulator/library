...
public class tWatchdog extends tSystem {
	...
	private static Thread softWatchdog = new Thread(){
		...
		while((fed && !dead) || !DriverStation.getInstance().isEnabled()){
		    ...
		    try{sleep((long)(expiration*expirationScale));} catch(InterruptedException e){Simulator.err(tWatchdog.class, softWatchdog, "The Watchdog was interrupted when it was trying to sleep, HOW RUDE!");}
		}
		if(!immortal){ dead = true; Simulator.getInstance().setStatus(Simulator.Status.WATCHDOG_NOT_FED);}
